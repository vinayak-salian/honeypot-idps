...
import sys # Added for path appending
...
try:
    ...
    print("[!] Error: geo_utils.py not found. Please ensure it is in the project folder.")
...
# Whitelist: Logged for history/stats, but stays off the map (via geo_utils) 

# and doesn't spam the console.
...
def analyze_and_log(src_ip): 
    ...
    is_suspicious = (num_unique >= PORT_THRESHOLD and scan_rate > 1.0)
    ...
    if (attack_type == "PortScan" and ml_confidence > 0.4) or is_suspicious:
        ...
        # This returns (None, None) for local IPs to hide them from heatmap

        # and real coordinates for botnets.
        ...
        try:
            ...
            if src_ip not in WHITELIST:
                ...
                print(f"[ðŸš¨ ALERT] PORT SCAN LOGGED: {src_ip} | Conf: {final_conf} | Ports: {num_unique}")
